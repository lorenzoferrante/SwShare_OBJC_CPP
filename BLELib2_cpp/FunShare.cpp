...
//					se q=1 la funzione ritorna il numero di byte da inviare
//					se q=2 la funzione ritorna il numero di byte che il dispositivo deve rispondere dopo la chiamata
int32_t prepare_data_for_get_set_time (api_query_t *query, api_answer_t *answer, uint32_t * time, uint32_t * cnt_attiv, uint32_t * time_attiv, char * nome, uint32_t *device_overfull, uint8_t get_set, uint8_t q)
{
	int32_t l;
	...
	if (q == 1)
	{
		...
		memcpy(query->data.get_set_time.nome, nome, sizeof(query->data.get_set_time.nome));
	...
	else if (q == 0)
	{
		...
		*device_overfull = answer->data.get_set_time.device_overfull;
		return l;
	}
	else
		return attesi;
